...
func RunCase(args []string, stdout io.Writer, stderr io.Writer) error {
	...
	fs.Usage = func() {
		fmt.Fprintf(stderr, "Usage: aard case --complaint FILE --out-dir DIR\n\n")
		fs.PrintDefaults()
	}
	...
	if strings.TrimSpace(*judgmentStandard) != "" {
		policy.JudgmentStandard = strings.TrimSpace(*judgmentStandard)
	}
