...
func TestClerkRunArgsUseServiceLauncherInterface(t *testing.T) {
	s := &Server{cfg: Config{
		AardBin:        "/opt/carve/aard",
		AardWorkingDir: "/opt/carve",
	}}
	args := s.clerkRunArgs(ClerkCreateRequest{}, "case-1", "run-1", "/tmp/case-1", "ex1")

	wantPrefix := []string{
		"--aard-bin", "/opt/carve/aard",
		"--case-id", "case-1",
		"--run-id", "run-1",
		"--out-dir", "/tmp/case-1",
	}
	...
	wantTail := []string{"--aard-working-dir", "/opt/carve", "ex1"}
...
if [ -z "$out_dir" ] || [ -z "$run_id" ] || [ -z "$input_prefix" ] || [ -z "$exec_ami" ] || [ -z "$aard_bin" ]; then
  exit 64
fi
...
printf 'AARD_BIN=%s\nAARD_INPUT_MODE=%s\nINPUT_PREFIX=%s\nOUTPUT_PREFIX=%s\nEXEC_AMI=%s\nCOMPLAINT=%s\nFILES=%s\nCASE_ID=%s\n' "$aard_bin" "$input_mode" "$input_prefix" "$output_prefix" "$exec_ami" "$complaint" "$files" "$case_id" > "$out_dir/run.env"
