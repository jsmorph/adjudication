...
func (s *Server) startClerkCase(req ClerkCreateRequest) (ClerkRecord, error) {
	...
	if execution != nil && execution.Mode == clerkExecutionAttested {
		var err error
		commandPath, args, err = attestedClerkCommand(req, rec, outDir, s.cfg.AardBin)
		if err != nil {
			s.unreserveClerkCase(caseID)
			return ClerkRecord{}, err
		}
	...
	if err := cmd.Start(); err != nil {
		...
		close(rec.done)
		return ClerkRecord{}, errors.Join(fmt.Errorf("start child: %w", err), persistErr, closeLogs())
	}
	s.mu.Lock()
...
func (s *Server) clerkRunArgs(req ClerkCreateRequest, caseID string, runID string, outDir string, example string) []string {
	...
	addString("--aard-working-dir", s.cfg.AardWorkingDir)
...
func (s *Server) markClerkFailed(rec *ClerkRecord, message string) error {
	...
	if err := s.persistClerkRecord(rec); err != nil {
		s.markClerkPersistenceFailed(rec, err)
		return fmt.Errorf("persist clerk record: %w", err)
	}
	return nil
}
