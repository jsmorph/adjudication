...
func validateAttestedClerkRequest(req ClerkCreateRequest) error {
	...
	if req.JudgmentStandard != "" {
		fields = append(fields, "judgment_standard")
	}
...
func attestedClerkCommand(req ClerkCreateRequest, rec *ClerkRecord, outDir string, aardBin string) (string, []string, error) {
...
func buildAttestedClerkUpdate(rec *ClerkRecord, exitCode int) attestedClerkUpdate {
	...
	att := &ClerkAttestationRecord{
		...
		InputPrefix:  firstNonEmpty(env["INPUT_PREFIX"], env["AARD_INPUT_PREFIX"], cfg.InputPrefix),
		OutputPrefix: firstNonEmpty(env["OUTPUT_PREFIX"], env["AARD_OUTPUT_PREFIX"], cfg.OutputPrefix),
	...
	successDir := filepath.Join(outDir, "aard-output")
	partialDir := filepath.Join(outDir, "aard-partial")
	successArchive := filepath.Join(outDir, "aard-output.tar.gz")
	partialArchive := filepath.Join(outDir, "aard-partial.tar.gz")
	if isDir(successDir) {
	...
	if att.LocalOutputDir == "" {
		return attestedClerkUpdate{attestation: att, err: "attested execution completed without extracted aard-output"}
	}
...
func clerkEffectiveOutputDir(rec ClerkRecord) string {
	if rec.Execution != nil && rec.Execution.Mode == clerkExecutionAttested {
		...
		successDir := filepath.Join(rec.OutDir, "aard-output")
		if isDir(successDir) {
			return successDir
		}
		partialDir := filepath.Join(rec.OutDir, "aard-partial")
		if isDir(partialDir) {
			return partialDir
		}
	}
...
func clerkAttestedOutputPrefix(rec ClerkRecord) (string, error) {
	...
	return firstNonEmpty(env["OUTPUT_PREFIX"], env["AARD_OUTPUT_PREFIX"], recordPrefix, cfg.OutputPrefix), nil
}
