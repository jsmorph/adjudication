...
work_root="${ARB_EXEC_WORK_ROOT:-/var/lib/arbattest-aar}"
...
if [ -z "$run_id" ]; then
    case "$mode" in
        aar)
            case "$aar_input_mode" in
                example) run_id="aar-$aar_example-$stamp" ;;
                *) run_id="aar-case-$stamp" ;;
            esac
            ;;
        *) run_id="run-$stamp" ;;
    esac
...
download_case_packet() {
    packet_name="${AAR_CASE_PACKET:-case.tar.gz}"
    manifest_name="${AAR_CASE_MANIFEST:-case-packet.json}"
    case "$packet_name" in
        ""|.*|*/*|*..*)
            echo "error: invalid AAR_CASE_PACKET: $packet_name" >&2
            exit 1
            ;;
    esac
    case "$manifest_name" in
        ""|.*|*/*|*..*)
            echo "error: invalid AAR_CASE_MANIFEST: $manifest_name" >&2
            exit 1
            ;;
    esac
    expected_packet_sha384="${AAR_CASE_PACKET_SHA384:-}"
    expected_manifest_sha384="${AAR_CASE_MANIFEST_SHA384:-}"
    ...
    aar)
        ...
        case "$aar_input_mode" in
            ...
            case-packet)
                ...
                ;;
            *)
                echo "error: unsupported AAR_INPUT_MODE: $aar_input_mode" >&2
                exit 1
                ;;
        esac
...
cat > "$manifest" <<EOF
{
  ...
  "input_mode": "$(json_string "$aar_input_mode")",
  "aar_example": "$(json_string "$aar_example")",
  "aar_case_id": "$(json_string "$aar_case_id")",
