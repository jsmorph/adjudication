...
PARTIAL_OBJECTS = {"run.log", "adc-partial.tar.gz"}
DEFAULT_PCR12 = "0" * 96
...
def stage_case_packet(args: argparse.Namespace, run_id: str, progress_log: Path) -> None:
    ...
    remote_tmp = f"/tmp/run-adc-attested-input-{run_id}-{os.getpid()}"
...
def build_remote_exec_command(args: argparse.Namespace, run_id: str, output_prefix: str) -> str:
    remote_runner = f"{args.remote_attest_dir.rstrip('/')}/run-adc.sh"
    exec_env_vars = [
        "INPUT_PREFIX",
        "IMAGE_TAR_S3",
        "ADC_INPUT_MODE",
        ...
        "ADC_CASE_PACKET",
        "ADC_CASE_MANIFEST",
        "ADC_CASE_PACKET_SHA384",
        "ADC_CASE_MANIFEST_SHA384",
        "RUN_ID",
        "OUTPUT_PREFIX",
    ]
    env = {
        ...
        "ADC_INPUT_MODE": args.input_mode,
        "ADC_CASE_ID": args.case_id or "",
        "ADC_CASE_PACKET": args.case_packet,
        "ADC_CASE_MANIFEST": args.case_manifest,
        "ADC_CASE_PACKET_SHA384": args.case_packet_sha384,
        "ADC_CASE_MANIFEST_SHA384": args.case_manifest_sha384,
        "RUN_ID": run_id,
...
def download_artifacts(args: argparse.Namespace, run_id: str, output_prefix: str, out_dir: Path, objects: set[str], progress_log: Path) -> set[str]:
    remote_tmp = f"/tmp/run-adc-attested-{run_id}-{os.getpid()}"
    remote = "\n".join(
        [
            "set -eu",
...
def extract_archives(out_dir: Path, status: str) -> None:
    if status == "success":
        safe_extract(out_dir / "adc-output.tar.gz", out_dir / "adc-output")
    elif status == "partial":
        safe_extract(out_dir / "adc-partial.tar.gz", out_dir / "adc-partial")
...
def verify_manifest_and_archive(args: argparse.Namespace, out_dir: Path, run_id: str, output_prefix: str, verification_log: Path) -> None:
    ...
    archive_path = out_dir / "adc-output.tar.gz"
    ...
    checks = [
        ...
        ("mode", manifest.get("mode") == "adc"),
        ("input_mode", manifest.get("input_mode") == args.input_mode),
        ("input_prefix", manifest.get("input_prefix") == args.input_prefix),
        ("adc_case_id", manifest.get("adc_case_id") == (args.case_id or "")),
        ("output_prefix", manifest.get("output_prefix") == output_prefix),
        ("archive_key", manifest.get("adc_archive_key") == output_prefix.rstrip("/") + "/adc-output.tar.gz"),
        ("run.log sha384", sha384_file(run_log_path) == manifest.get("log_sha384")),
        ("archive sha384", sha384_file(archive_path) == manifest.get("adc_archive_sha384")),
        ("archive bytes", str(archive_path.stat().st_size) == manifest.get("adc_archive_bytes")),
...
def write_run_env(args: argparse.Namespace, out_dir: Path, run_id: str, output_prefix: str) -> None:
    values = {
        "ADC_INPUT_MODE": args.input_mode,
        "ADC_CASE_ID": args.case_id or "",
        "ADC_COMPLAINT": args.complaint or "",
        "ADC_CASE_PACKET": args.case_packet,
        "ADC_CASE_PACKET_SHA384": args.case_packet_sha384,
        "ADC_CASE_MANIFEST": args.case_manifest,
        "ADC_CASE_MANIFEST_SHA384": args.case_manifest_sha384,
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument("--output-root", default=os.environ.get("OUTPUT_ROOT", "s3://agentcourt-data/arbattest/adc-runs"))
    ...
    p.add_argument("--adc-bin", default=os.environ.get("ADC_BIN") or shutil.which("adc") or "adc")
...
def main(argv: list[str]) -> int:
    try:
        ...
        run_name = args.case_id or "case"
        run_id = args.run_id or f"adc-{run_name}-{utc_stamp()}"
