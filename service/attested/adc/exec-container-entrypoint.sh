...
mode="${ADC_EXEC_MODE:-attest-only}"
work_root="${ADC_EXEC_WORK_ROOT:-/var/lib/arbattest-adc}"
stamp="$(date -u +%Y%m%dT%H%M%SZ)"
output_prefix="${OUTPUT_PREFIX:?OUTPUT_PREFIX is required}"
input_prefix="${INPUT_PREFIX:-}"
adc_input_mode="case-packet"
...
upload_adc_archive() {
    ...
    create_adc_archive "$src_dir" "$archive"
    set -- $(sha384sum "$archive")
    adc_archive_sha384="$1"
    set -- $(wc -c < "$archive")
    adc_archive_bytes="$1"
    adc_archive_key="$output_prefix/$object_name"
    aws s3 cp "$archive" "$adc_archive_key" --no-progress
}
...
download_case_packet() {
    packet_name="${ADC_CASE_PACKET:-case.tar.gz}"
    manifest_name="${ADC_CASE_MANIFEST:-case-packet.json}"
    case "$packet_name" in
        ""|.*|*/*|*..*)
            echo "error: invalid ADC_CASE_PACKET: $packet_name" >&2
            exit 1
            ;;
    esac
    case "$manifest_name" in
        ""|.*|*/*|*..*)
            echo "error: invalid ADC_CASE_MANIFEST: $manifest_name" >&2
            exit 1
            ;;
    esac
    expected_packet_sha384="${ADC_CASE_PACKET_SHA384:-}"
    expected_manifest_sha384="${ADC_CASE_MANIFEST_SHA384:-}"
...
case "$mode" in
    attest-only)
        ...
        ;;
    adc)
        : "${INPUT_PREFIX:?INPUT_PREFIX is required for ADC_EXEC_MODE=adc}"
        secrets_dir="$run_dir/secrets"
        adc_out="$run_dir/adc"
        event_upload_interval="${ADC_EVENTS_UPLOAD_INTERVAL_SECONDS:-60}"
        case "$event_upload_interval" in
            ""|0|*[!0-9]*)
                echo "error: invalid ADC_EVENTS_UPLOAD_INTERVAL_SECONDS: $event_upload_interval" >&2
                exit 1
                ;;
        esac
        mkdir -p "$secrets_dir" "$adc_out"
        ...
        set -- /usr/local/bin/adc-run-entrypoint \
            --out-dir "$adc_out" \
            --openclaw-auth codex \
        ...
        case "$adc_input_mode" in
            case-packet)
                ...
                ;;
            *)
                echo "error: unsupported ADC_INPUT_MODE: $adc_input_mode" >&2
                exit 1
                ;;
        esac
        events_path="$adc_out/events.ndjson"
        ...
        set -e
        next_event_upload=0
        while kill -0 "$adc_pid" 2>/dev/null; do
            now_epoch="$(date -u +%s)"
            if [ "$now_epoch" -ge "$next_event_upload" ]; then
                if ! upload_events_if_changed "$events_path" "$events_key" "$events_stamp_file"; then
                    ...
                    if kill -0 "$adc_pid" 2>/dev/null; then
                        if ! kill "$adc_pid" 2>/dev/null; then
                            printf 'error: failed to terminate adc after events upload failure\n' >> "$events_error_file"
                        fi
                    fi
                    break
                fi
        ...
        if [ "$events_failed" -ne 0 ] && [ "$adc_status" -eq 0 ]; then
            adc_status=1
        fi
        if [ "$adc_status" -ne 0 ]; then
            aws s3 cp "$log" "$output_prefix/run.log" --no-progress
            if ! upload_adc_archive "$adc_out" "adc-partial.tar.gz"; then
                echo "error: failed to upload partial ADC archive after adc exit status $adc_status" >&2
                exit 1
            fi
            ...
            echo "error: adc failed with exit status $adc_status" >&2
            exit "$adc_status"
        fi
        if ! upload_adc_archive "$adc_out" "adc-output.tar.gz"; then
            aws s3 cp "$log" "$output_prefix/run.log" --no-progress
            echo "error: failed to upload ADC archive" >&2
            exit 1
        fi
        ;;
    *)
        echo "error: unsupported ADC_EXEC_MODE: $mode" >&2
        exit 1
        ;;
esac
...
cat > "$manifest" <<EOF
{
  ...
  "input_mode": "$(json_string "$adc_input_mode")",
  "adc_case_id": "$(json_string "$adc_case_id")",
  ...
  "adc_archive_key": "$(json_string "$adc_archive_key")",
  "adc_archive_sha384": "$(json_string "$adc_archive_sha384")",
  "adc_archive_bytes": "$(json_string "$adc_archive_bytes")",
  "container_image_id": "$(json_string "${ADC_EXEC_IMAGE_ID:-}")",
  "container_image_tar_sha384": "$(json_string "${ADC_EXEC_IMAGE_TAR_SHA384:-}")",
