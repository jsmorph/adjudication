...
input_root="${INPUT_ROOT:-s3://agentcourt-data/arbattest/aard-inputs}"
output_root="${OUTPUT_ROOT:-s3://agentcourt-data/arbattest/aard-runs}"
out_root="${OUT_ROOT:-$service_root/out/aard-attested}"
...
run_id="${RUN_ID:-aard-$example-$stamp}"
...
uv run "$script_dir/run-arbd-attested.py" \
  --example "$example" \
