...
PARTIAL_OBJECTS = {"run.log", "aard-partial.tar.gz"}
DEFAULT_PCR12 = "0" * 96
...
def stage_case_packet(args: argparse.Namespace, run_id: str, progress_log: Path) -> None:
    ...
    remote_tmp = f"/tmp/run-arbd-attested-input-{run_id}-{os.getpid()}"
...
def build_remote_exec_command(args: argparse.Namespace, run_id: str, output_prefix: str) -> str:
    remote_runner = f"{args.remote_attest_dir.rstrip('/')}/run-aard.sh"
    exec_env_vars = [
        "INPUT_PREFIX",
        "IMAGE_TAR_S3",
        "AARD_INPUT_MODE",
        ...
        "AARD_CASE_PACKET",
        "AARD_CASE_MANIFEST",
        "AARD_CASE_PACKET_SHA384",
        "AARD_CASE_MANIFEST_SHA384",
        "RUN_ID",
        "OUTPUT_PREFIX",
    ]
    env = {
        ...
        "AARD_INPUT_MODE": args.input_mode,
        "AARD_EXAMPLE": args.example or "",
        "AARD_CASE_ID": args.case_id or "",
        "AARD_CASE_PACKET": args.case_packet,
        "AARD_CASE_MANIFEST": args.case_manifest,
        "AARD_CASE_PACKET_SHA384": args.case_packet_sha384,
        "AARD_CASE_MANIFEST_SHA384": args.case_manifest_sha384,
        "RUN_ID": run_id,
...
def download_artifacts(args: argparse.Namespace, run_id: str, output_prefix: str, out_dir: Path, objects: set[str], progress_log: Path) -> set[str]:
    remote_tmp = f"/tmp/run-arbd-attested-{run_id}-{os.getpid()}"
    remote = "\n".join(
        [
            "set -eu",
...
def extract_archives(out_dir: Path, status: str) -> None:
    if status == "success":
        safe_extract(out_dir / "aard-output.tar.gz", out_dir / "aard-output")
    elif status == "partial":
        safe_extract(out_dir / "aard-partial.tar.gz", out_dir / "aard-partial")
...
def verify_manifest_and_archive(args: argparse.Namespace, out_dir: Path, run_id: str, output_prefix: str, verification_log: Path) -> None:
    ...
    archive_path = out_dir / "aard-output.tar.gz"
    ...
    checks = [
        ...
        ("aard_case_id", manifest.get("aard_case_id") == (args.case_id or "")),
        ("output_prefix", manifest.get("output_prefix") == output_prefix),
        ("archive_key", manifest.get("aard_archive_key") == output_prefix.rstrip("/") + "/aard-output.tar.gz"),
        ("run.log sha384", sha384_file(run_log_path) == manifest.get("log_sha384")),
        ("archive sha384", sha384_file(archive_path) == manifest.get("aard_archive_sha384")),
        ("archive bytes", str(archive_path.stat().st_size) == manifest.get("aard_archive_bytes")),
    ...
    if args.input_mode == "example":
        checks.append(("aard_example", manifest.get("aard_example") == args.example))
    else:
        checks.extend(
            [
...
def write_run_env(args: argparse.Namespace, out_dir: Path, run_id: str, output_prefix: str) -> None:
    values = {
        ...
        "AARD_EXAMPLE": args.example or "",
        "AARD_CASE_ID": args.case_id or "",
        "AARD_COMPLAINT": args.complaint or "",
        "AARD_CASE_PACKET": args.case_packet,
        "AARD_CASE_PACKET_SHA384": args.case_packet_sha384,
        "AARD_CASE_MANIFEST": args.case_manifest,
        "AARD_CASE_MANIFEST_SHA384": args.case_manifest_sha384,
...
def default_parser_path() -> Path:
    ...
    candidates = [
        Path.cwd() / "attest" / "parse_attestation.py",
        script_path.parents[3] / "attest" / "parse_attestation.py",
        script_path.with_name("parse_attestation.py"),
    ]
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument("--output-root", default=os.environ.get("OUTPUT_ROOT", "s3://agentcourt-data/arbattest/aard-runs"))
    ...
    p.add_argument("--image-tar-s3", default=os.environ.get("IMAGE_TAR_S3", "s3://agentcourt-data/arbattest/images/arbd-glue-poc.tar"))
...
def main(argv: list[str]) -> int:
    try:
        ...
        run_id = args.run_id or f"aard-{run_name}-{utc_stamp()}"
