...
Shared core code now consists of model request parsing, model access, persona loading, proof-document tools, and the small runtime pool used by direct juror and council execution.  The Go module names the model client and SQLite as its direct third-party dependencies.  The remaining package inventory contains only `adc`, `arb`, `arbd`, and `common` core packages.
