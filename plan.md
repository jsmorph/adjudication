...
- [x] If D7 retains attested operation, move the attested Python and shell programs, Docker definitions, S3 configuration, AMI and PCR handling, service routes, tests, and runbooks to `service`.
...
- [x] Change service container builds to consume explicit core source or prebuilt artifacts from a pinned `carve` revision.  All three builds consume full `CORE_COMMIT` and `SERVICE_COMMIT` values.
- [x] Verify one local packet build, one container build, and one attested-driver test before deleting the original files from `carve`.  ARB passed these checks at `service@dc7c61d61b478dd2bf24fdc7e1e4924d80b37443` against `carve@a4cc40d99899721d957c4f370040998306f771e9`; AARD passed at `service@5bf19d083ea670cdbf767ccf304c81ec712f90d4` against `carve@489cd9ecdb46f41f036d825fc9f5221f68c13c1a`; ADC passed at `service@58bd12061e604cca551ddcebcf9e76e46dea3098` against `carve@755246b50d0726dfc8dce9faf6a852b3a31f8b18`.
- [x] On `carve`, retain the core `case-packet` commands and delete Dockerfiles, `attest/`, attested tools, S3 settings, and attested runbooks after their service-owned replacements pass.
- [x] Search both branches for stale source paths and implicit assumptions that service and core files share one checkout.  Current source and operator documentation contain no removed implementation imports, combined commands, host-specific checkout paths, or shared-parent assumptions; historical paths remain only in development journals.
...
AARD attested execution moved to `service/attested/arbd` in service commit `6a4441006b1f81ae573e9754c76e3c3f185d9e34`.  The corrected image at `service@5bf19d083ea670cdbf767ccf304c81ec712f90d4` fetched and verified the full core and service revisions, then built the installed `aard`, `aardengine`, and `aard-run` programs.  Docker produced image `sha256:a35e3e6bdd8090f890ab3cdf9d877fbcc758670cce1b72b5ad25c56243d83264`, whose service entrypoint, launcher help, and installed core complaint validation passed.

ADC attested execution moved to `service/attested/adc` in service commit `58bd12061e604cca551ddcebcf9e76e46dea3098`.  Its image fetched and verified the full core and service revisions, then built the installed `adc`, `adcengine`, and `adc-run` programs.  Docker produced image `sha256:89b6e85a6de9525bc8402e82b14ecc0fabe9102f4bd38e166819a28059cae938`, whose service entrypoint, launcher help, and installed core packet construction passed; service commit `9766451` records the corrected proof and runbook.
...
- [x] Check every local Markdown link and command example on both branches.  Service has 50 valid local links and 73 valid shell blocks.  Carve has 251 present local targets, two ADC inputs generated by `examples/ex1/sign.sh`, and 59 valid shell blocks; all retained core help topics and all nine service command help paths pass.
